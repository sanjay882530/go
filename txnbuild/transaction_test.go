...
func TestBuildChallengeTx(t *testing.T) {
	...
	{
		// 1 minute timebound
		tx, err := BuildChallengeTx(kp0.Seed(), kp0.Address(), "testwebauth.hcnet.org", "testanchor.hcnet.org", network.TestNetworkPassphrase, time.Minute)
		assert.NoError(t, err)
	...
	{
		// 5 minutes timebound
		tx, err := BuildChallengeTx(kp0.Seed(), kp0.Address(), "testwebauth.hcnet.org", "testanchor.hcnet.org", network.TestNetworkPassphrase, time.Duration(5*time.Minute))
		assert.NoError(t, err)
		txeBase64, err := tx.Base64()
		assert.NoError(t, err)
...
func TestReadChallengeTx_validSignedByServerAndClient(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	readTx, readClientAccountID, _, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.hcnet.org", []string{"testanchor.hcnet.org"})
...
func TestReadChallengeTx_invalidOperationNoSourceAccount(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, _, err = ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.hcnet.org", []string{"testanchor.hcnet.org"})
...
func TestReadChallengeTx_doesVerifyHomeDomainFailure(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, _, err = ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.hcnet.org", []string{"willfail"})
...
func TestReadChallengeTx_matchesHomeDomain(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, matchedHomeDomain, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.hcnet.org", []string{"testanchor.hcnet.org"})
	require.NoError(t, err)
...
func TestReadChallengeTx_doesNotMatchHomeDomain(t *testing.T) {
	...
	tx64, err := tx.Base64()
	require.NoError(t, err)
	_, _, matchedHomeDomain, err := ReadChallengeTx(tx64, serverKP.Address(), network.TestNetworkPassphrase, "testwebauth.hcnet.org", []string{"not", "going", "to", "match"})
