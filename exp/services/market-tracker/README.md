# Hcnet Market Tracker

The Hcnet Market Tracker allows you to monitor the spreads of any desired asset pairs, and makes them available for Prometheus to scrape.
...
- Environment variables: `HCNET_EXPERT_AUTH_KEY` and `HCNET_EXPERT_AUTH_VAL`, the authentication header for Hcnet Expert; `RATES_API_KEY` and `RATES_API_VAL`, the key-value pair for the OpenExchangeRates API. Note that the exact format of these variables may change, as we finalize internal deployment.
