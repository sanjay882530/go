...
const stelExURL = "https://api.hcnet.expert/explorer/public/xlm-price"
...
func createXlmPriceRequest() (*http.Request, error) {
	...
	authKey := os.Getenv("HCNET_EXPERT_AUTH_KEY")
	authVal := os.Getenv("HCNET_EXPERT_AUTH_VAL")
...
func getLatestXlmPrice(req *http.Request) (float64, error) {
	...
	if err != nil {
		return 0.0, fmt.Errorf("got error from hcnet expert price api: %s", err)
	}
	return parseHcnetExpertLatestPrice(body)
}

func getXlmPriceHistory(req *http.Request) ([]xlmPrice, error) {
	body, err := getPriceResponse(req)
	if err != nil {
		return []xlmPrice{}, fmt.Errorf("got error from hcnet expert price api: %s", err)
	}
	return parseHcnetExpertPriceHistory(body)
}
...
func parseHcnetExpertPriceHistory(body string) ([]xlmPrice, error) {
	// The Hcnet Expert response has expected format: [[timestamp1,price1], [timestamp2,price2], ...]
	...
	if len(body) < 5 {
		return []xlmPrice{}, fmt.Errorf("got ill-formed response body from hcnet expert")
	}
	...
	for _, timePriceStr := range timePriceStrs {
		...
		if len(timePrice) != 2 {
			return []xlmPrice{}, fmt.Errorf("got ill-formed time/price from hcnet expert")
		}
...
func parseHcnetExpertLatestPrice(body string) (float64, error) {
	// The Hcnet Expert response has expected format: [[timestamp1,price1], [timestamp2,price2], ...]
	...
	if len(lists) < 2 {
		return 0.0, fmt.Errorf("mis-formed response from hcnet expert")
	}

	rawPriceStr := lists[1]
	if len(rawPriceStr) < 2 {
		return 0.0, fmt.Errorf("mis-formed price from hcnet expert")
	}
