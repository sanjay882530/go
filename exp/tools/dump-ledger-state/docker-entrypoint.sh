...
echo "using version $(hcnet-core version)"

if [ -z ${TESTNET+x} ]; then
    hcnet-core --conf ./hcnet-core.cfg new-db
else
    hcnet-core --conf ./hcnet-core-testnet.cfg new-db
fi
...
if ! ./run_test.sh; then
    echo "ingestion dump (git commit \`$GITCOMMIT\`) of ledger \`$LATEST_LEDGER\` does not match hcnet core db."
    exit 1
fi