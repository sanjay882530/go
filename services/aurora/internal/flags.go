...
const (
	...
	captiveCoreMigrationHint = "If you are migrating from Aurora 1.x.y, start with the Migration Guide here: https://developers.hcnet.org/docs/run-api-server/migrating/"
)
...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			...
			FlagDefault: "",
			Required:    false,
			Usage:       "path to hcnet core binary (--remote-captive-core-url has higher precedence). If captive core is enabled, look for the hcnet-core binary in $PATH by default.",
		...
		&support.ConfigOption{
			...
			FlagDefault: true,
			Required:    false,
			Usage:       "causes Aurora to ingest from a Captive Hcnet Core process instead of a persistent Hcnet Core database",
		...
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "causes Aurora to enable the experimental Ingestion Filtering and the ingestion admin HTTP endpoint at /ingestion/filter",
...
func NewAppFromFlags(config *Config, flags support.ConfigOptions) (*App, error) {
	...
	if config.HcnetCoreURL == "" {
		return nil, fmt.Errorf("flag --%s cannot be empty", HcnetCoreURLFlagName)
	}
	if config.Ingest && !config.EnableCaptiveCoreIngestion && config.HcnetCoreDatabaseURL == "" {
		return nil, fmt.Errorf("flag --%s cannot be empty", HcnetCoreDBURLFlagName)
	}
...
func ApplyFlags(config *Config, flags support.ConfigOptions, options ApplyOptions) error {
	...
	if config.Ingest {
		...
		if config.EnableCaptiveCoreIngestion {
			...
			binaryPath := viper.GetString(HcnetCoreBinaryPathName)

			// If the user didn't specify a Hcnet Core binary, we can check the
			// $PATH and possibly fill it in for them.
			if binaryPath == "" {
				if result, err := exec.LookPath("hcnet-core"); err == nil {
					binaryPath = result
					viper.Set(HcnetCoreBinaryPathName, binaryPath)
			...
			if binaryPath == "" && config.RemoteCaptiveCoreURL == "" {
				return fmt.Errorf("Invalid config: captive core requires that either --%s or --remote-captive-core-url is set. %s",
					HcnetCoreBinaryPathName, captiveCoreMigrationHint)
			}
			...
			if config.RemoteCaptiveCoreURL == "" && (binaryPath == "" || config.CaptiveCoreConfigPath == "") {
				if options.RequireCaptiveCoreConfig {
					...
					errorMessage := fmt.Errorf(
						...
						HcnetCoreBinaryPathName, CaptiveCoreConfigPathName, captiveCoreMigrationHint,
					)
					...
					case network.TestNetworkPassphrase:
						...
						config.HistoryArchiveURLs = []string{"https://history.hcnet.org/prd/core-testnet/core_testnet_001/"}
					case network.PublicNetworkPassphrase:
						configFileName = "captive-core-pubnet.cfg"
						config.HistoryArchiveURLs = []string{"https://history.hcnet.org/prd/core-live/core_live_001/"}
			...
			if config.HcnetCoreURL == "" && config.RemoteCaptiveCoreURL == "" && config.CaptiveCoreToml.HTTPPort != 0 {
				config.HcnetCoreURL = fmt.Sprintf("http://localhost:%d", config.CaptiveCoreToml.HTTPPort)
				viper.Set(HcnetCoreURLFlagName, config.HcnetCoreURL)
			}
		}
	} else {
		if config.EnableCaptiveCoreIngestion && (config.CaptiveCoreBinaryPath != "" || config.CaptiveCoreConfigPath != "") {
			...
			return fmt.Errorf("Invalid config: one or more captive core params passed (--%s or --%s) but --ingest not set. "+captiveCoreMigrationHint,
				HcnetCoreBinaryPathName, captiveCoreConfigFlag)
		}
		if config.HcnetCoreDatabaseURL != "" {
			return fmt.Errorf("Invalid config: --%s passed but --ingest not set. ", HcnetCoreDBURLFlagName)
		}
